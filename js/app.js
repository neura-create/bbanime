...
(function () {
  ...
  function drawFrame(index) {
    ...
    // Fondo ambiente: version diminuta del mismo frame estirada a pantalla
    // completa (efecto difuminado sin costo de un filtro blur real). Rellena
    // todo el canvas sin bordes duros ni recuadros, cualquiera sea la
    // proporcion de la pantalla.
    ambientCtx.drawImage(img, 0, 0, ambientCanvas.width, ambientCanvas.height);
    ctx.drawImage(ambientCanvas, 0, 0, cw, ch);
    ctx.fillStyle = "rgba(11, 10, 31, 0.35)";
    ctx.fillRect(0, 0, cw, ch);

    // Primer plano: la botella COMPLETA siempre visible ("contain"), con un
    // margen arriba para que el header nunca tape el sombrero.
    var availH = ch - TOP_SAFE;
    var scale = Math.min(cw / iw, availH / ih);
    var dw = iw * scale, dh = ih * scale;
    var dx = (cw - dw) / 2;
    var dy = TOP_SAFE + (availH - dh) / 2;
